...
################################################################################################################################################################
##### 7. Caclulate cluster markers and plot potential marker gene expression
...
DimPlot(killi.combined.filt, label = TRUE, group.by = "Annotation_v1", repel = T, cols = as.vector(P22)) + NoLegend()
dev.off()


pdf(paste(Sys.Date(),"Killifish_Tissues_3_cohortsUMAP_Annotation_v1_plot_CLEAN_withlegend.pdf", sep = "_"), height = 5, width = 10)
DimPlot(killi.combined.filt, label = F, group.by = "Annotation_v1", repel = T, cols = as.vector(P22))
dev.off()

write.table(cbind("col" = P22, "cell_type" = sort(unique(killi.combined.filt$Annotation_v1))), file = paste0(Sys.Date(),"_color_palette_annotation.txt"), sep = "\t", row.names = F)


################    summary UMAPs for paper ################
# By biological group
pdf(paste(Sys.Date(),"Killifish_Tissues_3_cohortsSinglets_UMAP_color_by_Condition_postRPCA_CleanQCAnnotatedONLY.pdf", sep = "_"), height = 5, width = 5.8)
DimPlot(killi.combined.filt, reduction = "umap", group.by = "Group", shuffle = T)
dev.off()

# By sex
my.umap.colors.sex <- c(alpha("deeppink"    , alpha = 0.4 ) ,
                        alpha("deepskyblue" , alpha = 0.4 ) )
pdf(paste(Sys.Date(),"Killifish_Tissues_3_cohortsSinglets_UMAP_color_by_Sex_postRPCA_CleanQCAnnotatedONLY.pdf", sep = "_"), height = 5, width = 5.8)
DimPlot(killi.combined.filt, reduction = "umap", group.by = "Sex", shuffle = T, cols = my.umap.colors.sex)
dev.off()

# By Batch
my.umap.colors.group <- c(alpha("firebrick3"    , alpha = 0.4 ) ,
                          alpha("darkturquoise" , alpha = 0.4 ) ,
                          alpha("goldenrod3"    , alpha = 0.4 ) )
pdf(paste(Sys.Date(),"Killifish_Tissues_3_cohortsSinglets_UMAP_color_by_Batch_postRPCA_CleanQCAnnotatedONLY.pdf", sep = "_"), height = 5, width = 5.8)
DimPlot(killi.combined.filt, reduction = "umap", group.by = "Batch", shuffle = T, cols = my.umap.colors.group)
dev.off()

# By Tissue
pdf(paste(Sys.Date(),"Killifish_Tissues_3_cohortsSinglets_UMAP_color_by_Tissue_postRPCA_CleanQCAnnotatedONLY.pdf", sep = "_"), height = 5, width = 5.8)
DimPlot(killi.combined.filt, reduction = "umap", group.by = "Tissue", shuffle = T)
dev.off()
