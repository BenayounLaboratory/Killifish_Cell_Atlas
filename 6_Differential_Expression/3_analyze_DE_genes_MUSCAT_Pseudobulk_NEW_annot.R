...
rownames(up.genes)[apply(up.genes,1,sum)>4]
...
################
hpx.data <- matrix(0,length(counts.pb),6)
rownames(hpx.data) <- names(vst.counts)
colnames(hpx.data) <- colnames(vst.counts[[1]])

for (i in 1:length(counts.pb)) {
  hpx.data[i,] <- counts.pb[[i]]["hpx",]/colSums(counts.pb[[i]])
}
...
pdf(paste0(Sys.Date(),"_hpx_expression_raw.pdf"))
Heatmap(scale_rows(hpx.data), 
        col = colorRamp2(c(-2,0,2), c("darkblue", "white", "red"), transparency = 0, space = "LAB"),
        border = T, rect_gp = gpar(col = "grey", lwd = 0.5) ,
        cluster_rows = F,
        cluster_columns = F,
        column_title = "Raw hpx expression")
dev.off()


################
LOC107373896.data <- matrix(0,length(counts.pb),6)
rownames(LOC107373896.data) <- names(vst.counts)
colnames(LOC107373896.data) <- colnames(vst.counts[[1]])

for (i in 1:length(counts.pb)) {
  LOC107373896.data[i,] <- counts.pb[[i]]["LOC107373896",]/colSums(counts.pb[[i]])
}


pdf(paste0(Sys.Date(),"_LOC107373896_expression_raw.pdf"))
Heatmap(scale_rows(LOC107373896.data), 
        col = colorRamp2(c(-2,0,2), c("darkblue", "white", "red"), transparency = 0, space = "LAB"),
        border = T, rect_gp = gpar(col = "grey", lwd = 0.5) ,
        cluster_rows = F,
        cluster_columns = F,
        column_title = "Raw LOC107373896 expression")
dev.off()
